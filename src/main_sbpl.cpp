...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()("help", "produce help message")(
      ...
      "input file (yaml)")("primitives,p",
                           po::value<std::string>(&primitivesFile)->required(),
                           "primitive file (prim)")(
      "stats", po::value<std::string>(&statsFile)->default_value("stats.yaml"),
      "output file (yaml)")("output,o",
  ...
  // which planning is done) this is for the default level - base level
  ...
  for (int x = 0; x < width; ++x) {
    for (int y = 0; y < height; ++y) {
      ...
      bpcm_env->collide(co_robot.get(), &collision_data,
  ...
  bool r = environment_navxythetalat.InitializeEnv(
      width, height, mapdata.data(), startx, starty, starttheta, goalx, goaly,
      goaltheta,
      /*goaltol_x*/ 0, /*goaltol_y*/ 0, /*goaltol_theta*/ 0, perimeterptsV,
      cellsize_m,
      /*nominalvel_mpersecs*/ sqrt(0.5),
      /*timetoturn45degsinplace_secs*/ sqrt(0.5 * M_PI_2),
      /*obsthresh*/ 100, primitivesFile.c_str());
  ...
  ARAPlanner *planner =
      new ARAPlanner(&environment_navxythetalat, /*bforwardsearch*/ true);
  ...
  printf("done planning with cost %d (suboptimality is: %f, desired: %f)\n",
         cost, planner->compute_suboptimality(), planner->get_solution_eps());
  ...
  environment_navxythetalat.PoseDiscToCont(x_d, y_d, theta_d, x_c, y_c,
                                           theta_c);
  sbpl_xy_theta_pt_t pt(x_c, y_c, theta_c);
  xythetaPath.push_back(pt);

  std::vector<EnvNAVXYTHETALATAction_t> actions;
  environment_navxythetalat.GetActionsFromStateIDPath(&solution_stateIDs_V,
                                                      &actions);
  ...
  for (const auto &state : xythetaPath) {
    out << "      - [" << state.x << "," << state.y << ","
        << std::remainder(state.theta, 2 * M_PI) << "]" << std::endl;
  }
  out << "    actions_mprim:" << std::endl;
  for (const auto &action : actions) {
    out << "      - [" << (int)action.starttheta << "," << (int)action.dX << ","
        << (int)action.dY << "," << (int)action.endtheta << "]" << std::endl;
  }
