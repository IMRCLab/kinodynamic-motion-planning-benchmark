...
def run_sbpl(filename_env, folder):
    ...
    if result.returncode != 0:
        ...
        for start_c, dX, dY, end_c in result["result"][0]["actions_mprim"]:
            found = False
            for m in motions:
                if m['prim']['start_c'] == start_c and (m['prim']['endpose_c'] == [dX, dY, end_c] or m['prim']['endpose_c'] == [
                                                        dX, dY, end_c - 16] or m['prim']['endpose_c'] == [dX, dY, end_c + 16]):
                    actions.extend(m['actions'])
                    found = True
                    break
            if not found:
                print(
                    "ERROR: couldn't find corresponding motion!",
                    start_c,
                    dX,
                    dY,
                    end_c)
